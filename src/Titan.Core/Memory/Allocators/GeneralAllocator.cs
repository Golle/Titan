...
/// </summary>
public unsafe class GeneralAllocator : IGeneralAllocator
{
    ...
    //NOTE(Jens): Add these methods and structs when we need them

    //[MethodImpl(MethodImplOptions.AggressiveInlining)]
    //public TitanBuffer AllocateBuffer(int size, bool initialize = false)
    //{
    //    Debug.Assert(size >= 0);
    //    return AllocateBuffer((uint)size, initialize);
    //}

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public TitanBuffer AllocateBuffer(uint size, bool initialize = false)
    {
        var mem = Allocate(size, initialize);
        return new TitanBuffer(mem, size);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public TitanArray<T> AllocateArray<T>(uint count, bool initialize = false) where T : unmanaged
    {
        var mem = Allocate<T>(count, initialize);
        return new TitanArray<T>(mem, count);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryAllocateArray<T>(out TitanArray<T> array, uint count, bool initialize = false) where T : unmanaged
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public TitanQueue<T> AllocateQueue<T>(uint count) where T : unmanaged
    {
        var mem = Allocate<T>(count);
        return new TitanQueue<T>(mem, count);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryAllocateQueue<T>(out TitanQueue<T> queue, uint count) where T : unmanaged
    {
        ...
        queue = new TitanQueue<T>(mem, count);
        return true;
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T* Allocate<T>(uint count, bool initialize = true) where T : unmanaged => (T*)Allocate((uint)sizeof(T) * count, initialize);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T* Allocate<T>(bool initialize = true) where T : unmanaged => (T*)Allocate((uint)sizeof(T), initialize);

    public void* Allocate(uint size, bool initialize = true)
    {
        Debug.Assert(size > 0);
