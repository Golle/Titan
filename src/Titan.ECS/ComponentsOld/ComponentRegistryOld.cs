...
internal class ComponentRegistryOld : IDisposable
{
    ...
    public ComponentRegistryOld(WorldConfigurationOld config)
    {
        _config = config;
        _worldId = config.Id;
        Logger.Trace<ComponentRegistryOld>("Register Components");
        foreach (var (type, poolType, count) in config.Components)
        {
            Register(type, poolType, count);
        }
        Logger.Trace<ComponentRegistryOld>("Register Components Finished");
    }
    ...
    public void Register(Type componentType, ComponentPoolTypes poolType, uint numberOfComponents = 0)
    {
        ...
        Logger.Trace<ComponentRegistryOld>($"Added a {poolType} component pool for type {componentType.Name} with a max capacity {count}");
